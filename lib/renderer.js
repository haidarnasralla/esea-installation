...
export function renderFlickerOut(ctx, snippet, progress, flickerParams, effects, config) {
  ...
  // Visibility is purely random each frame, weighted by progress
  // Early: almost always visible. Late: rarely visible.
  // Intensity controls how quickly it becomes unreliable
  const visibilityThreshold = baseVisibility * (1 - intensity * 0.3);
  const isVisible = Math.random() < visibilityThreshold;
  
  if (!isVisible) {
    // Maybe render a ghost/residue flash occasionally
    if (progress > 0.3 && Math.random() < intensity * 0.08) {
      renderGhostFlash(ctx, snippet, config);
    }
    return;
  }
  
  // Calculate jitter offset (only when visible, keeps it grounded)
  const jitterAmount = jitter * progress * progress;
