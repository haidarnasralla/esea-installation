...
function generateSAMSamples(word, voiceParams = null) {
  ...
  try {
    ...
    console.warn('SAM failed for word:', word, e);
    return null;
  }
}
...
export function generatePhraseSamples(text, options = {}) {
  ...
  for (const word of words) {
    ...
    if (samples) {
      ...
      // Add resampled audio (avoid spread operator to prevent stack overflow)
      for (let i = 0; i < resampled.length; i++) {
        allSamples.push(resampled[i]);
      }
...
export function init() {
  console.log(`Hybrid TTS ready (LPC voices: ${LPC_VOICE_NAMES.join(', ')}, SAM fallback)`);
}
